...
- [ ] Day 3: the fine-tune. Read `docs/02_EVALUATION_SPEC.md` §3 first, then
  `training/mine_negatives.py` -- **train split only**, and `assert_no_shared_chunks`
  in `goldset/split.py` explains why a chunk that is gold for a test question
  must never be sampled as a train negative. Lane 6 is already wired:
  `lanes.lane_six_factory()` builds it, and it enters `REGISTRY` once the
  checkpoint is on the Hub.

Carried into Day 3 from Day 2:
- **The lane-3 fusion gate ties rather than passes on the test split** (30/35
  vs BM25's 30/35, different questions). Investigated and accepted as
  small-sample noise: fusion wins 68/80 on train and 98/115 overall. Not
  resolved by tuning `rrf_k`, which would fit the test split. See
  `python -m backend.sweep_rrf` and the Known Limitations entry.
- **Groq decommissioned `llama-3.1-8b-instant`.** `GROQ_MODEL` is now
  `openai/gpt-oss-20b`, which is a *reasoning* model: it bills reasoning
  tokens and emits them before any content. The Day 3 groundedness judge uses
  the same setting and will hit the same trap -- budget max_tokens above the
  reasoning spend and set `reasoning_effort="low"`, or it returns empty
  content with `finish_reason="length"`.
- The dense lanes' ~450 ms p95 is the network round-trip to hosted Supabase,
  not pgvector. Retrieval quality is unaffected; the latency column is not
  comparable to a co-located deployment.
