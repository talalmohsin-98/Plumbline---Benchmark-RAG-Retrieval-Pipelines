...
- **Lane 5 is the one lane whose numbers are not bit-reproducible.** Everything else in this project is deterministic given the corpus and the seed. HyDE calls a hosted LLM, and batched GPU inference is not bit-reproducible even at `temperature=0`. Two back-to-back runs over the identical test split returned identical recall@5 and recall@10 but MRR 0.6286 vs 0.6310 and cost $5.043e-05 vs $5.025e-05. Read lane 5's third decimal place as noise; the retrieval numbers held.
